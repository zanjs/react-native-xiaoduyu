...
export class MyEditor extends React.Component {
  ...
  componentDidMount() {
    ...
    RNMessageChannel.on('initial-content', contentJSON => {
      ...
      self.setState({
        editorState: EditorState.createWithContent(convertFromRaw(JSON.parse(contentJSON)), decorator)
      })

    })

    RNMessageChannel.on('add-photo', text => {
      self.addImage(text)
    })
